...
- [ ] The five section `README.md` files (`Marble Language Reference`, `Base Server Reference`, `Analytics Reference`, `Other Model Types Reference`, `Deployment and Installation Reference`) contain ` ```dataview ` code blocks that rendered as live queries in Obsidian but render as inert/plain code on the published site, since Dataview is an Obsidian-only plugin. Either rewrite these sections as static content/links, or find a Quartz-side equivalent.
...
- [ ] Add a link on the site (header or footer) back to the source repo on GitHub (https://github.com/phbronson999/ifs-dev-docs), so visitors can get from the published pages to the actual repo.
- [ ] Add a way to export the published site as a local Obsidian vault — e.g. a downloadable zip of `content/`'s markdown (with wikilinks/frontmatter intact) so a visitor can drop it straight into their own vault instead of only reading it as a website.
